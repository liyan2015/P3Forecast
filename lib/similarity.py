...
LastEditTime: 2024-07-29 01:34:23
Description: calculate the similarity between two time series
(1) fastpdtw: pattern-aware Dynamic Pattern Warping class based on dtw
(2) fastdtw: Dynamic Pattern Warping
(3) mmd_rbf: Maximum Mean Discrepancy using rbf (gaussian) kernel
'''
from lib.utils import np
...
def fastpdtw(x, y, radius=1, dist=None):
    ''' return the approximate distance between 2 time series with O(N)
        ...
        normalized distance : float
            normalized distance
    '''
    x2, y2 = np.atleast_2d(x), np.atleast_2d(y)
    if x2.shape[0] == 1:
        x2 = x2.T
    if y2.shape[0] == 1:
        y2 = y2.T
    x, y, dist = __prep_inputs(x2, y2, dist)
    distance,path = __fastdtw(x, y, radius, dist)
    return distance , distance/max(len(x),len(y)) # 1-distance/max(len(x),len(y))
...
def __dtw(x, y, window, dist):
    ...
    D[0, 0] = (0, 0, 0)
    xdf = np.insert(np.diff(x,axis=0),0,0)
    ydf = np.insert(np.diff(y,axis=0),0,0)

    for i, j in window:
        if xdf[i-1]*ydf[j-1]>0 or abs( xdf[i-1]-ydf[j-1])<=0.001:
            # if xdf[i-1] == 0 and ydf[j-1] == 0:
            #     dt = 0
            # else:
            #     dt = abs(xdf[i-1]-ydf[j-1])/(abs(xdf[i-1]+ydf[j-1])/2)
            dt = abs(xdf[i-1]-ydf[j-1])
        else:
            # if x[i-1] == 0 and y[j-1] == 0:
            #     dt = 0
            # else:
            #     dt = abs(x[i-1]-y[j-1])/((abs(x[i-1])+abs(y[j-1]))/2)
            dt = abs(x[i-1]-y[j-1])
        # dt = 1-np.exp(-dt)
